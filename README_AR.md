...
## تحديث Hossam AI
تم تحديث `main.py` و`rules_ar.txt` بحيث:
- يحلل وصف الحالة كاملًا بدل الاعتماد على كلمات منفردة.
- يطلب المعلومات الناقصة قبل تحديد العقوبة عندما تكون مؤثرة.
- يميز بين الحالات التطبيقية المحددة والقواعد الأساسية.
- يمنع اختراع مواد أو عقوبات غير موجودة في ملف القوانين.
- يبقي واجهة Android كما هي: `POST /chat` يستقبل `message` ويرجع `answer`.
